/* $Id: PlayerDieer.java,v 1.32 2012/09/02 22:27:37 kiheru Exp $ */
/***************************************************************************
...
public class PlayerDieer {
	...
	public void onDead(final Entity killer) {
		...
		if (numberOfDrops > 0) {
			...
			if (ringList.isEmpty()) {
				final int hpLeft = (int) Math.round(player.getBaseHP() - player.getHP());
				player.sendPrivateText(NotificationType.NEGATIVE, "Twoje zwłoki zawierają " + Grammar.enumerateCollection(strings) + ", ale możesz odzyskać " + Grammar.itthem(numberOfDrops) + ". Ocaliłeś aż " + skillPercentage + "% swojch dotychczasowych umiejętności oraz straciłeś " + hpLeft + " punkt" + " życia!");
			} else {
				final int hpLeft = (int) Math.round(player.getBaseHP() - player.getHP());
				player.sendPrivateText(NotificationType.NEGATIVE, "Twoje zwłoki zawierają " + Grammar.enumerateCollection(strings) + ", ale możesz odzyskać " + Grammar.itthem(numberOfDrops) + ". Ocaliłeś aż " + skillPercentage + "% doświadczenia oraz straciłeś " + hpLeft + " punkt" + " życia!");
			}
		} else {
