/* $Id: FlowerGrowerTransFormer.java,v 1.3 2010/09/19 02:22:47 nhnb Exp $ */
/***************************************************************************
...
public class FlowerGrowerTransFormer implements Transformer {
	...
	public RPObject transform(final RPObject object) {
			...
			String itemview = object.get("class");
			//itemname = itemname.substring(itemname.lastIndexOf('/') + 1, itemname.length() - "_grower".length());
			itemview = itemview.substring(itemview.lastIndexOf('/') + 1, itemview.length() - "_grower".length());
			return new FlowerGrower(object, itemname, itemview);
	}
