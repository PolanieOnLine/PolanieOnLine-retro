...
public class StendhalRPZone extends MarauroaRPZone {
	...
	private synchronized void add(final RPObject object, final Player player, final boolean expire) {
		...
		if (object instanceof Blood) {
			...
			playersAndFriends.add((Owczarek) object);
		} else if (object instanceof OwczarekPodhalanski) {
			playersAndFriends.add((OwczarekPodhalanski) object);
	...
	private static String translateZoneName(final String zoneName) {
		...
		if (m.matches()) {
			...
			if ("int".equals(level)) {
				...
				if (levelValue < -2) {
					result = "głęboko pod ziemią na poziomie ";
				} else {
				...
				if (levelValue != 0) {
					if (levelValue > 1) {
						result = "wysoko nad powierzchnią ziemi na poziomie ";
					} else {
						result = "na poziomie ";
					}
				} 
			...
			final String[] directions = new String[] { ".+_n\\d?e\\d?($|_).*",
					"północny-wschód ", "_n\\d?e\\d?($|_)", "_",
					".+_n\\d?w\\d?($|_).*", "północny-zachód ", "_n\\d?w\\d?($|_)",
					"_", ".+_s\\d?e\\d?($|_).*", "południowy-wschód ",
					"_s\\d?e\\d?($|_)", "_", ".+_s\\d?w\\d?($|_).*",
					"południowy-zachód ", "_s\\d?w\\d?($|_)", "_", ".+_n\\d?($|_).*",
					"północ ", "_n\\d?($|_)", "_", ".+_s\\d?($|_).*", "południe ",
					"_s\\d?($|_)", "_", ".+_w\\d?($|_).*", "zachód ",
					"_w\\d?($|_)", "_", ".+_e\\d?($|_).*", "wschód ",
					"_e\\d?($|_)", "_", ".+_c\\d?($|_).*", "centrum ",
					"_c\\d?($|_)", "_",};
