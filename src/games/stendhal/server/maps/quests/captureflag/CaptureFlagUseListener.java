...
public class CaptureFlagUseListener implements UseListener {
	...
	private boolean checkOnline() {
		if ((target == null) || (target.isGhost() && (sender.getAdminLevel() < AdministrationAction.getLevelForCommand("ghostmode")))) {
			sender.sendPrivateText("Brak wojownika zwanego \"" + targetName + "\".");
			return false;
		}
		return true;
	}
	...
	public boolean onUsed(RPEntity user) {
		...
		if (effect.equals("not-equipped")) {
			user.sendPrivateText("Nie możesz oznaczyć dopóki nie masz łuku i specjalnej amunicji");
			return false;
		}
		
		if (effect.equals("not-in-range")) {
			user.sendPrivateText("Musisz być w zasięgu wojownika, aby go oznaczyć");
			return false;
		}
		...
		if (effect.equals("fumble") || effect.equals("drop") || effect.equals("")) {
			...
			result = "powinien przyspieszyć - 0.1";
