/* $Id: AdministrationAction.java,v 1.38 2012/09/02 11:40:50 kiheru Exp $ */
/***************************************************************************
	...
	public static boolean isPlayerAllowedToExecuteAdminCommand(final Player player,
		...
		if (adminlevel < required.intValue()) {
			...
			if (verbose) {
				...
				if (adminlevel == 0) {
					player.sendPrivateText("Przepraszam, ale musisz być administratorem, aby wykonać \""
							+ command + "\".");
				} else {
					player.sendPrivateText("Posiadasz "
							+ adminlevel + " poziom administratora, a wymagany jest " + required
							+ " poziom, aby skorzystać z komendy \"" + command + "\".");
				}
			}
			return false;
		}
