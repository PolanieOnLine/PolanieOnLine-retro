		...
		if (hasNeededAttributes(action)) {
			...
			if ("name".equals(stat) && (changed instanceof Player)) {
				logger.error("DENIED: Admin " + player.getName() + " trying to change player " + action.get(TARGET)
						+ "'s name");
				player.sendPrivateText("Nazwa nie może zostać zmieniona.");
				return;
			}

			if (ADMINLEVEL.equals(stat)) {
				player.sendPrivateText("Użyj #/adminlevel #<imie wojownika> #[<nowy poziom>], aby wyświetlić lub zmienić poziom administratora.");
				return;
			}

			if (TITLE.equals(stat) && (changed instanceof Player)) {
				player.sendPrivateText("Nazwa atrybutu może nie być zmieniona od razu.");
				return;
			}
			...
			if (type == null) {
				player.sendPrivateText("Atrybut, który zmieniasz nie jest zdefiniowany w RPClass("
				...
				if (isParsableByInteger(type)) {
					...
					if (mode.equalsIgnoreCase(SET)) {
						...
						if (type.getType() != Type.FLAG) {
							player.sendPrivateText("Atrybut, który chcesz odznaczyć nie jest typu 'flag'.");
							return;
						}
