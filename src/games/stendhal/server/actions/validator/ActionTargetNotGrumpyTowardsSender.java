/* $Id: ActionTargetNotGrumpyTowardsSender.java,v 1.2 2012/09/13 20:46:30 nhnb Exp $ */
/***************************************************************************
...
public class ActionTargetNotGrumpyTowardsSender implements ActionValidator {
	...
	public String validate(Player player, RPAction action, ActionData data) {
		...
		if (!targetPlayer.containsKey("buddies", player.getName())) {
			if (grumpy.length() == 0) {
				return playerName + " zamknął się i szuka spokoju od przyjaciół";
			} else {
				return playerName + " szuka samotności od wszystkich z wyjątkiem najbliższych przyjaciół: " + grumpy;
			}
		}
