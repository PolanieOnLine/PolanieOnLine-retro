/* $Id: IgnoreAction.java,v 1.8 2011/04/02 15:44:20 kymara Exp $ */
/***************************************************************************
		...
		if (action.has(LIST)) {
			...
			if (player.getSlot("!ignore").size() > 0) {
				...
				if (namesAndReasons.size() < 0) {
					...
					player.sendPrivateText("Twoja lista ignorowanych jest pusta.");
				} else if (namesAndReasons.size() < 50) {
					player.sendPrivateText("Na Twojej liście ignorowanych znajduje się " + Grammar.enumerateCollection(namesAndReasons) + ".");
				} else {
					// list could be unfeasibly long to print out to player so give a sublist but also the total number out of interest
					player.sendPrivateText("Ignorujesz " + namesAndReasons.size() + " wojowników. Pierwsze 50 wpisów na Twojej liście ignorowanych " 
			...
			if (player.addIgnore(who, duration, reason)) {
				player.sendPrivateText(who + " został dodany do listy ignorowanych.");
			}
		}
