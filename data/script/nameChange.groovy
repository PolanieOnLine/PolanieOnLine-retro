...
if (player != null) {
	if (args[0].equals("remove")) {
		player.remove("title");
		player.sendPrivateText("Twoje oryginalne imiê zosta³o odzyskane. Zmieñ obszar, aby zadzia³a³y zmiany.");
		player.update();
		player.notifyWorldAboutChanges();
	} else {
		player.put("title", args[0]);
		player.sendPrivateText("Twoje imiê zosta³o zmienione na " + player.get("alternateTitle") + ".");
		player.update();
		player.notifyWorldAboutChanges();
	}
}
